...
2025‑06‑16 rev.4 — **defaults**:
• Usa **Picamera2** (libcamerasrc) sem precisar de flag.
• HEF padrão **yolov8s_pose.hef** (procura no diretório do script).
• Mantém --show para preview; quem quiser fonte USB pode usar --usb.
"""
...
def pulse_led(dur=3.0):
    if GPIO_OK:
        LED.set_value(1)
        t = threading.Timer(dur, lambda: LED.set_value(0)); t.daemon = True; t.start()
...
class ArmsCallback(app_callback_class):
    ...
    def _save(self):
        ...
        for f in list(self.buf): vw.write(f); vw.release(); print(f"[SAVE] {path} ({len(self.buf)}f)")
    def handle_buffer(self, pad, info):
        buf = info.get_buffer(); frm = get_numpy_from_buffer(buf); self.buf.append(frm.copy())
        kpts = []
        meta = buf.get_meta("HailoJsonMeta")
        if meta:
            import json; obj = json.loads(meta.get_json()).get("objects", [])
            if obj: kpts=[(p['x'],p['y']) for p in max(obj,key=lambda o:o['confidence'])['keypoints']]
        crossed = self.det(kpts)
        txt = "ARMS CROSSED" if crossed else "ARMS NOT CROSSED"
        cv2.putText(frm, txt, (30,40), cv2.FONT_HERSHEY_SIMPLEX,1.2,(0,0,255) if crossed else (0,255,0),3)
        if self.preview_q: self.preview_q.put_nowait(cv2.cvtColor(frm, cv2.COLOR_RGB2BGR))
...
class PreviewThread(threading.Thread):
    def __init__(self,q): super().__init__(daemon=True); self.q=q; self.stop=threading.Event()
    def run(self):
        while not self.stop.is_set():
            try: frm=self.q.get(timeout=0.1); cv2.imshow("Pose",frm);
            except queue.Empty: continue
            if cv2.waitKey(1)&0xFF==27: self.stop.set();
...
def main():
    ...
    ap.add_argument("--usb", action="store_true", help="Use /dev/video0 webcam instead of Picamera2")
    args=ap.parse_args()

    if not os.path.exists(args.hef):
        raise FileNotFoundError(f"HEF '{args.hef}' não encontrado")

    q = queue.Queue(maxsize=2) if args.show else None
    user_data = ArmsCallback(args.buffer*args.fps, args.fps, q)
    ...
    pv=PreviewThread(q) if args.show else None
    if pv: pv.start()
    try: app.run()
    except KeyboardInterrupt: pass
    finally: app.stop();
    if pv: pv.stop.set(); pv.join()
